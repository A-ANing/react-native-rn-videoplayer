        ...
        this._panResponder = PanResponder.create({
            ...
            onPanResponderMove: (evt, gestureState) => {
                ...
                if (this.recordHandeY.length === 10) {
                    ...
                    //判断前10个滑动坐标 来确定用户是想操作音量还是进度 避免斜着滑
                    //（防止：比如上下滑动是操作音量 但是我左右平行滑动只想调节播放进度 但是滑动一会再以斜坡的方式往上滑也会调节音量）
                    //这样做的目的就是 知道了用户是想改变音量还是播放进度  从而达到哪怕用户上下滑动一会了 再斜着滑动还是只会调节音量 不会去调节播放进度
                    //只有用户松手了 才会做下次判定 只要用户不松手 哪怕乱滑  滑圆圈也只会操作用户想操作的那一项
                        ...
                        if (Math.abs(this.moveYData) > 5) {
                            ...
                            if (Math.abs(this.BrightnessY) <= 1 && this.startX < this.state.width / 2) {
                                ...
                                if (this.BrightnessY) {
                                    ...
                                    /**视频亮度调节
                                     * 
                                     * 安卓不是直接修改系统亮度，而是修改当前Activity亮度（rn来说就是只修改当前app的亮度，不处于该app时还是系统亮度）
                                     * 如果修改系统亮度，则要获取允许修改系统设置的权限，
                                     * 这个权限在安卓6以上必须要手动进入手机的设置页面，用户自己打开，
                                     * 如果修改当前Activity亮度则不需要获取任何权限。
                                     * 
                                    **/
                                    //console.log("this.brightnessData",this.brightnessData)
                                    Platform.OS==="android"?
...
if(!this.state.showOpenVip){
    ...
    onLoad = (data) => {
        ...
        // this.hide.start(() => this.setState({ showConts: false })); // 开始执行动画
        //console.log("进度", this.playDotX)
        //console.log("缓冲进度", this.playDotX)
    }
       ...
       const spin = this.spinValue.interpolate({
        ...
        return (
            <View>
                ...
                {
                    this.state.width != width ? null
                        :
                        (Platform.OS === "android" ? <View style={{ zIndex: 999, backgroundColor: "#000000", height: StatusBar.currentHeight, }}></View> : <SafeAreaView style={{ backgroundColor: "#000" }} />)
                }
